...
def fetch_all_data(period: str = "5y") -> pd.DataFrame:
    ...
    """
    logger.info(f"Fetching Cotton (CT=F), USD Index (DX-Y.NYB), and 10Y Yield (^TNX) for the last {period}...")
    try:
        ...
        merged_df = pd.concat([cotton_close, dxy_close, tnx_close], axis=1)
        
        # Forward fill any missing days (e.g. if one market was closed but another was open)
        merged_df.ffill(inplace=True)
        # Drop rows where Cotton was completely unavailable
